...
2. **日历自定义拦截 (v1.2.0+)**：用户可在日历多选界面，针对特变日期强制设置“强班/强休”，并可为“强班”单独赋予专有闹钟时间，记录持久化在 Room 数据库中。
3. **日常程序触发**：用户在三星“日常程序”中设定触发条件（如：每晚 22:00），并添加“RoutinePlus”提供的静态快捷方式「执行闹钟规则」。
4. **静默执行**：触发时，应用后台会先读取并拦截自定义日期配置。若无覆盖记录，则根据 DataStore 全局配置和法定节假日算法进行综合判定。如为工作日（或强班），则自动调用系统底层 API 并设置系统闹钟，全程无 UI 打扰。
